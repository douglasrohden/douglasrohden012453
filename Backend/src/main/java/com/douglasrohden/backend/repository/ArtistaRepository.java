...
public interface ArtistaRepository extends JpaRepository<Artista, Long> {
    ...
    @Query(value = "SELECT a.id, a.nome, a.genero, a.imageUrl, COUNT(al) " +
            "FROM Artista a LEFT JOIN a.albuns al " +
            "WHERE (:q = '' OR lower(a.nome) LIKE lower(concat('%', :q, '%'))) " +
            "GROUP BY a.id",
            countQuery = "SELECT COUNT(a) FROM Artista a WHERE (:q = '' OR lower(a.nome) LIKE lower(concat('%', :q, '%')))"
    )
    Page<Object[]> searchWithAlbumCount(@Param("q") String q, Pageable pageable);
}
