...
export const ToastProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <ToastContext.Provider value={{ addToast, updateToast, removeToast }}>
      {children}
      {toasts.length > 0 && (
        <div className="pointer-events-none fixed inset-0 z-[9990] bg-gray-900/50 transition-opacity duration-300 dark:bg-black/80" />
      )}
      <div className="fixed bottom-5 left-5 z-[9999] flex flex-col-reverse gap-3">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className="translate-x-0 transform opacity-100 transition-all duration-300 ease-in-out"
            style={{ animation: "slideIn 0.4s ease-out forwards" }}
          >
            <Toast
              className="overflow-hidden border-l-4 border-transparent shadow-2xl dark:bg-gray-800"
              style={{
                borderLeftColor:
                  toast.type === "success"
                    ? "#10B981"
